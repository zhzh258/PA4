...
# The default scene. This scene is to demo the normal mapping
class SceneOne(Component, Animation):
    ...
    def __init__(self, shaderProg):
        ...
        m1 = Material(np.array((0.1, 0.1, 0.1, 0.1)), np.array((0.2, 0.2, 0.2, 1)),np.array((0.4, 0.4, 0.4, 0.1)), 64)
        sphere.setMaterial(m1)
        sphere.setTexture(shaderProg, "assets/normalmap.jpg")
        sphere.renderingRouting = "lighting"
        self.addChild(sphere)


        torus = Component(Point((1, 0, 0)), DisplayableTorus(shaderProg, 0.25, 0.5, 36, 36))
        m2 = Material(np.array((0.1, 0.1, 0.1, 0.1)), np.array((0.2, 0.2, 0.2, 1)), np.array((0.3, 0.3, 0.3, 1.0)), 64)
        torus.setMaterial(m2)
        torus.setTexture(shaderProg, "assets/normalmap.jpg")
